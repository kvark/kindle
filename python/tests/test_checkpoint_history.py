...
def fake_runner(tmp_path, monkeypatch):
    ...
    def run(*, history=False, steps=6, every=4, exploration=False, collision=None, fail_at=None, encoder=None):
        ...
        class Agent:
            ...
            def __init__(self, weights, streams, config, **kwargs):
                self.streams, self.config = streams, config
                case['encoder'] = kwargs.get('encoder')
...
def test_history_preserves_explicit_tiny_selection(fake_runner):
    run, _ = fake_runner
    case = run(history=True, encoder='levjepa-tiny')
    assert case['encoder'] == 'levjepa-tiny'
    assert [path.name for path in case['saves']] == ['4', '6']
    assert audit(case['output'])['budget_complete']
