//! Run the IRIS agent across multiple environments, hopping between them
...
use iris::envs::{cart_pole::CartPole, grid_world::GridWorld, mountain_car::MountainCar};
...
impl Envs {
    ...
    fn observe(&self, which: Which) -> iris::Observation {
        match which {
    ...
    fn step(&mut self, which: Which, action: &iris::Action) {
        match which {
            Which::Grid => {
...
fn main() {
    ...
    println!("IRIS Multi-Env (Grid → Cart → Mountain rotating)");
    println!("==================================================");
    ...
    let adapter = Box::new(GenericAdapter::discrete(
        0,
        iris::envs::grid_world::OBS_DIM,
        iris::envs::grid_world::NUM_ACTIONS,
    ));
    ...
    for step in 0..total_steps {
        ...
        if step > 0 && step % hop_every == 0 {
            let adapter: Box<dyn iris::EnvAdapter> = match which {
