...
| Pong | Historical 400k root 1009: 24/24 wins, mean +20.5833 versus −20.5417 untrained. New matched root 2017 training complete; frozen evaluation running. | ≥20 natural matches, ≥90% wins, mean ≥+15, no cutoffs. Finish the new three-root campaign; historical 1009 is not its third root. | [Historical trained](../runs/atari-recovered-confirmations-20260911.xPz5ud/pong/seed1009-evaluation.mp4), [control](../runs/atari-recovered-confirmations-20260911.xPz5ud/pong/seed1009-untrained-evaluation.mp4) |
...
1. Complete the [matched Pong campaign](../runs/pong-block-confirmation-20260916.rBwdGF/declaration.md):
   roots 2017, 3019, then fresh 1009. Per root: training, frozen final policy,
   fresh initialization, untrained frozen control, full replay and videos.
   Each phase is individually reviewed. No automatic follower or retry.
2. Qualify the prepared Breakout four-action adapter against the same eighteen-
   action runtime: gradients, complete state, initialization/restore, N6 pixels,
   replay and memory. Then declare a matched learning pilot changing only action
   vocabulary. Do not repeat old backend qualification unchanged.
3. Separately test Freeway and Qbert with continuous 400,008-action pilots and
...
4. Confirm successful changed recipes on all three fresh roots with controls.
...
for this campaign. The default editable extension is historical: select the
declared source-matched package. Cleanup does not change an active binary.
