...
/// PPO clipped-surrogate policy graph with end-to-end encoder. Combines
/// `build_ppo_policy_graph`'s clip with `build_policy_graph_e2e`'s
/// encoder-inside-the-graph. The trust region the PPO clip provides
/// is the natural fix for the post-solve crash observed with plain
/// e2e on CartPole — once the policy committed to a good policy and
/// scored +197, plain advantage-weighted CE kept pushing past it
/// until the policy crashed back. PPO clip caps the per-update
/// policy change, preventing that overshoot.
///
/// Inputs:
/// - `"obs"`: `[batch_size, obs_dim]`
/// - `"task"`: `[batch_size, task_dim]`
/// - `"action"`: `[batch_size, action_dim]` — plain one-hot taken action
/// - `"advantage"`: `[batch_size, 1]` — per-row scalar Â_t
/// - `"old_prob_taken"`: `[batch_size, 1]` — π_old(a_t|s_t)
/// - `"value_target"`: `[batch_size, 1]` — TD target for value head
///
/// Outputs: `[loss, logits, value]`.
///
/// Loss: `-mean(min(r_t·Â_t, clip(r_t, 1±ε)·Â_t)) + vf_coef·MSE(V, target) - β·H(π)`.
/// Entropy term uses `stop_gradient(z)` to keep its gradient out of
/// the encoder (same fix as the non-PPO e2e variant).
pub fn build_ppo_policy_graph_e2e(
    ...
    // π_new(a | s) — gather via softmax * one_hot followed by per-row
    // sum (matmul with [K, 1] ones).
    let sm_new = g.softmax(logits);
    let p_per_class = g.mul(sm_new, action);
    let ones_k1 = g.constant(vec![1.0; action_dim], &[action_dim, 1]);
    let new_prob_taken = g.matmul(p_per_class, ones_k1);
    ...
    // Clip ratio to [1-ε, 1+ε] via greater-gated select.
    let one_plus_eps = g.constant(vec![1.0 + clip_eps; batch_size], &[batch_size, 1]);
    let one_minus_eps = g.constant(vec![1.0 - clip_eps; batch_size], &[batch_size, 1]);
    let ones_bx1 = g.constant(vec![1.0; batch_size], &[batch_size, 1]);
