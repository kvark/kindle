...
fn check_behavior_gradients(action_count: usize) {
    assert_eq!(
        std::env::var("KINDLE_INIT_EXPECTED_DRIVER").as_deref(),
        Ok("580.178.04"),
        "this diagnostic requires an explicit current-driver declaration"
    );
    let config = DreamerConfig::new(action_count);
    ...
    assert_eq!(device.requested_device_id.as_deref(), Some("0x2c02"));
    assert!(!device.is_software_emulated);
    eprintln!(
        "behavior_device={}",
        serde_json::to_string(&device).unwrap()
    );
    memory(&gpu, "device.ready");
    ...
    for index in 0..partitions {
        ...
        part.step();
        part.wait();
        memory(&gpu, "partition.wait_returned");
...
fn memory(gpu: &blade_graphics::Context, phase: &str) {
    let stats = gpu.memory_stats();
    eprintln!(
        "behavior_memory={}",
        serde_json::json!({"phase": phase, "usage_bytes": stats.usage, "budget_bytes": stats.budget})
    );
